...
def run_demo():
    ...
    while True:
        ...
        # Wrap the vector into a GVector and then draw the GVector.
        GVector(vec=v).draw()

        # Draw v from the mouse cursor
        GVector(vec=v).draw(vmouse)

        # Add vmouse + v to create a new vector, and draw this one from the origin.
        v2 = vmouse + v
        GVector(vec=v2).draw()
